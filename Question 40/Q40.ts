...
let make_album = (artist_name: string, album_title : string) => {
...
let Make_Album = (artist_name: string, album_title : string, tracks? : number) => {
    if (tracks) return {Artist : artist_name, Album : album_title, Tracks : tracks};
    else return {Artist : artist_name, Album : album_title, Tracks: tracks??""};
